...
export function ChannelType({ channelId, loggedUser } : { channelId: number, loggedUser: User}) {
	...
	const updateChannel = useMutation({
		mutationFn: ([property, newValue]: [keyof Channel, string]) => updateChannelProperties(channelId, property ,newValue),
		onSuccess: () => { 
			...
			/>
			{
				...
				{ isEditing? 
					(
						...
						<p id="chan_header_changetype" >Mode: {(chanType) ? chanType : channel.mode} <FontAwesomeIcon onClick={() => setIsEditing(true)} icon={faPencil} id="fa_see_more"/></p>
					)
				}
